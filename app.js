...
const TRANSLATIONS = {
    es: {
        ...
        'log.load_complete': 'Carga completada: {n} clientes in total.',
        ...
        'log.project_warn': 'AVISO: No se pudo crear progetto per {client}: {msg}',
...
class TakeOffClient {
    ...
    async getGroups(users = []) {
            ...
            (user.groups || []).forEach(group => {
                if (group && group.id != null) {
                    groupsById.set(group.id, {
                        id: group.id,
                        name: group.name || group.displayName || `Group ${group.id}`
                    });
                }
            });
        });
        ...
        try {
            ...
            if (!response.ok) return [...groupsById.values()];
            const data = await response.json();
            const raw = data.value !== undefined ? data.value : data;
            const apiGroups = Array.isArray(raw) ? raw : (raw.data || []);
...
const App = {
    ...
    getDisplayName(item, fallbackPrefix = 'Elemento') {
        return item?.displayName ||
            `${item?.firstName || ''} ${item?.lastName || ''}`.trim() ||
            item?.name ||
            `${fallbackPrefix} ${item?.id ?? ''}`.trim();
    },

    buildAssignedEntityIds(assignees = this.selectedAssignees) {
        return [...new Set(assignees
            .filter(a => a && (a.type === 'user' || a.type === 'group') && Number.isInteger(parseInt(a.id)))
            .map(a => `${a.type === 'user' ? 1 : 2}_${parseInt(a.id)}`))];
    },
    ...
    async loadLookups() {
        try {
            ...
            const groupsList = await this.client.getGroups(usersList);
            ...
            if (me) {
                this.selectedAssignees.push({
                    id:   me.id,
                    type: 'user',
                    name: this.getDisplayName(me, 'Utente')
                });
            }
    ...
    async loadMaintenanceContacts() {
        ...
        try {
            ...
            // Bind contact search
            document.getElementById('contact-search-input').oninput = (e) => {
                this.renderContactList(e.target.value);
            };

            // Bind select/deselect visible buttons
            document.getElementById('btn-contacts-select-visible').onclick = () => {
                this.toggleVisibleContacts(true);
            };
            document.getElementById('btn-contacts-deselect-visible').onclick = () => {
    ...
    renderAssigneeDropdownList(filter = '') {
        ...
        if (users.length > 0) {
            ...
            users.forEach(u => {
                const name = this.getDisplayName(u, 'Utente');
                const isSelected = this.selectedAssignees.some(a => a.type === 'user' && a.id === u.id);
                html += `<div class="assignee-option${isSelected ? ' selected' : ''}" data-id="${u.id}" data-type="user" data-name="${this.escapeHtml(name)}">
                    <span class="assignee-option-check">${isSelected ? '✓' : ''}</span>
                    <span class="assignee-option-name">👤 ${this.escapeHtml(name)}</span>
                </div>`;
            });
        }
        ...
        if (groups.length > 0) {
            ...
            groups.forEach(g => {
                const name = this.getDisplayName(g, 'Gruppo');
                const isSelected = this.selectedAssignees.some(a => a.type === 'group' && a.id === g.id);
                html += `<div class="assignee-option${isSelected ? ' selected' : ''}" data-id="${g.id}" data-type="group" data-name="${this.escapeHtml(name)}">
                    <span class="assignee-option-check">${isSelected ? '✓' : ''}</span>
                    <span class="assignee-option-name">👥 ${this.escapeHtml(name)}</span>
                </div>`;
            });
    ...
    renderPreviewTable() {
        ...
        this.filteredTasks.forEach(task => {
            ...
            tr.innerHTML = `
                ...
                <td class="col-badge">
                    <span class="freq-badge freq-${this.escapeHtml(task.recurrence)}">${this.escapeHtml(task.recurrence)}</span>
                </td>
    ...
    async startBulkCreation() {
        ...
        const assignedEntityIds = this.buildAssignedEntityIds(this.selectedAssignees);
